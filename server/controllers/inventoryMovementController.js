...
export const getInventoryMovements = async (req, res) => {
    try {
        const inventoryMovements = await InventoryMovement.find().populate('productos.producto');
        if (inventoryMovements.length === 0) {
            return res.status(404).json({ error: 'No se encontraron movimientos de inventario' });
        }
...
export const getInventoryMovementById = async (req, res) => {
    try {
        ...
        const inventoryMovement = await InventoryMovement.findOne({ documentoTransaccion }).populate('productos.producto');
        if (!inventoryMovement) {
            return res.status(404).json({ error: 'No se encontró el movimiento de inventario' });
        }
...
export const createInventoryMovement = async (req, res) => {
    try {
        const { documentoTransaccion, fecha, tipoMovimiento, productos, descripcion, usuario } = req.body;

        if ((!documentoTransaccion || !fecha || !tipoMovimiento || !productos || !descripcion || !usuario)) {
        return res.status(400).json({ error: 'Todos los campos son obligatorios' });
        }

        const existingInventoryMovement = await InventoryMovement.findOne({ documentoTransaccion });
        if (existingInventoryMovement) {
            return res.status(400).json({ error: 'Ya existe un movimiento de inventario con este documento' });
        }
        ...
        if (!Array.isArray(productos) || productos.length === 0) {
            return res.status(400).json({ error: 'Debe incluir al menos un producto en el movimiento' });
        }

        //Actualizar el stock del producto
        await Promise.all(productos.map(async (item) => {
            if (!item.codigo || typeof item.cantidad !== 'number') {
                throw new Error('Cada producto debe incluir código y cantidad válidos');
            }
            ...
            if (tipo === 'entrada') {
                ...
                if (product.stock < item.cantidad) {
                    throw new Error(`No hay suficiente stock del producto ${product.nombre}`);
                }
                product.stock -= item.cantidad;
            }
        ...
        console.error(error);
        res.status(500).json({ error: 'Error al crear el movimiento de inventario' });
    }
};
...
export const updateInventoryMovement = async (req, res) => {
    try {
        ...
        const inventoryMovement = await InventoryMovement.findOne({ documentoTransaccion });
        if (!inventoryMovement) {
            return res.status(404).json({ error: 'No se encontró el movimiento de inventario' });
        }
        ...
        await Promise.all(inventoryMovement.productos.map(async (item) => {
            const product = await Product.findOne({ codigo: item.codigo });
            if (product) {
...
export const deleteInventoryMovement = async (req, res) => {
    try {
        ...
        await InventoryMovement.deleteOne({ documentoTransaccion: documentoTransaccion });
...
export const getInventoryMovementsByType = async (req, res) => {
    try {
        ...
        const inventoryMovements = await InventoryMovement.find({ tipoMovimiento: tipoMovimiento }).populate('productos.producto');
        if (inventoryMovements.length === 0) {
            return res.status(404).json({ error: 'No se encontraron movimientos de inventario' });
        }
